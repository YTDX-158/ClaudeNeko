...
- **对话变聪明**：claude 工作目录指向你的记忆目录，远程对话和桌面一样"认识用户"
- **安全**：配对码动态生成、HttpOnly Cookie 鉴权、远程下禁用「下载视频」防 SSRF；未配对访问只给配对页
- **移动端适配**：窄屏侧栏变抽屉，手机浏览器体验完整
- 需 `cloudflared`（可选，未装则仅局域网可用）
...
### 生成媒体配置（可选 · 技能包生图/生视频/下载转录）
技能包的生图 / 生视频 / 下载转录走**豆包 / 火山方舟**。可直接复用上面的视觉 Key（同一豆包账号），也可单独配：
...
| 技能包生图 / 生视频 / 下载转录 | + 豆包 / 火山方舟 Key（可用视觉 Key 代替，可选） |
| 远程访问（手机 / 公网） | + cloudflared（可选，未装仅局域网可用） |
...
4. 技能包「下载视频」用于转录/分析**你自己的素材**（自己的视频 / 录屏），勿下载他人版权内容
